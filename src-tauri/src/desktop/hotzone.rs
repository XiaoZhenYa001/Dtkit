...
pub fn is_in_hotzone(
    ...
    // 精确判断：鼠标必须在热区范围内
    mouse_x >= hotzone_left
        && mouse_x <= hotzone_right
        && mouse_y >= hotzone_top
        && mouse_y <= hotzone_bottom
}

/// 检查坐标是否在侧边栏区域内（动态计算，更宽松的判断用于保持显示）
pub fn is_in_panel_dynamic(
    mouse_x: i32,
    mouse_y: i32,
    primary_width: i32,
    primary_height: i32,
            ...
            while HOTZONE_RUNNING.load(Ordering::SeqCst) {
                ...
                if !is_panel_visible {
                    ...
                    if in_hotzone {
                        ...
                        } else if in_hotzone_since.unwrap().elapsed().as_millis()
                            >= config.trigger_delay as u128
                        {
                            // 触发显示
